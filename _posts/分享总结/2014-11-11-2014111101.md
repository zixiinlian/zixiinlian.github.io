...
1.通过*{margin:0;padding:0}重置不同浏览器下margin，padding不一致的问题
2.float元素同时设置margin在ie6下margin加倍，通过设置display：inline将其转化为行内元素结束
3.min-height兼容性问题，通过ie下设置height方式解决
...
6.火狐不识别background-position-x,background-position-y
7.两个块元素，竖向的margin不会增加，会叠加，间距为最大的margin
